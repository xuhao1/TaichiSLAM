...
sigma_Q = 0.02 #sigma of camera quaternion
...
max_landmark_pos = np.array([1.5, 1.5, 2])
min_landmark_pos = np.array([-1.5, -1.5, 0.5])
...
def generate_poses():
    ...
    for i in range(Np):
        ...
        qnoise = np.random.normal(qgt, sigma_Q, size=4)
...
def visual_sparse_map(qs, Ts, Ls, Lgt, ax=None, title="SparseMap", axis_len = 0.5):
    if ax is None:
        fig = plt.figure("SparseMap", figsize=(15,10))
        plt.tight_layout()
    ...
    ax.quiver(Ts[:,0], Ts[:,1], Ts[:,2], x_axis[:, 0], x_axis[:, 1], x_axis[:, 2], linewidths=-.5, arrow_length_ratio=0.,  color="red", label="x axes")
    ax.quiver(Ts[:,0], Ts[:,1], Ts[:,2], y_axis[:, 0], y_axis[:, 1], y_axis[:, 2], linewidths=-.5, arrow_length_ratio=0.,  color="green", label="y axes")
    ax.quiver(Ts[:,0], Ts[:,1], Ts[:,2], z_axis[:, 0], z_axis[:, 1], z_axis[:, 2], linewidths=-.5, arrow_length_ratio=0.,  color="blue", label="z axes")
    ax.legend()
...
if __name__ == "__main__":
    ...
    print("L_p.grad", L_p.grad.to_numpy()*1000)
